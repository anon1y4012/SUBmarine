...
- **Poster grid & list views** — searchable, filterable by service (vertical service-list menu with saved lists), with the overlap metric cards tucked behind a Stats page.
...
- **Seed-time protection** — optionally reads Cleanuparr's Download Cleaner rules and keeps any matched torrent that has not yet met the configured minimum seed time, so removals never cut a torrent short of your tracker rules.
